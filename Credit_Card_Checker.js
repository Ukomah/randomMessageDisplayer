...
//This function validateCred(), checks the validity of credit card number, 
//returns true when an array contains digits of a valid credit 
...
//This function findInvalidCards(), checks through the nested 
//array for which numbers are invalid, 
//and return another nested array of invalid cards.
function findInvalidCards(arrOfSerialDigits){
  let invalidDigits = [];
   for (let j = 0; j < arrOfSerialDigits.length  ; j++ ){
     let validate = validateCred(arrOfSerialDigits[j])
     if (validate === false){
       invalidDigits.push(arrOfSerialDigits[j]);
     }
   }
...
//idInvalidCardCompanies() returns an array of 
